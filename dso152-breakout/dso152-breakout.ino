...
#define AUTO_BUTTON  PC8  //K1 RIGHT_BUTTON 
#define MODE_BUTTON  PC7  //K2 
#define UP_BUTTON    PC6  //K3 
#define DOWN_BUTTON  PA9  //K4 
#define RUN_BUTTON  PA10  //K5 LEFT_BUTTON

#define POWER_BUTTON PC11
#define POWER_ON_Q4  PC10

uint16_t status_power=0;
...
game_type games[GAMES_NUMBER] =
  ...
  {
    { 10, 60, 8, 6, 40, 8, 8, 3, 5, { 0x18, 0x66, 0xFF, 0xDB, 0xFF, 0x7E, 0x24, 0x3C }, 28, -28 },
    { 10, 50, 8, 6, 40, 8, 8, 3, 5, { 0xFF, 0x99, 0xFF, 0xE7, 0xBD, 0xDB, 0xE7, 0xFF }, 20, -20 },
    { 10, 50, 8, 6, 40, 8, 8, 3, 5, { 0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55 }, 28, -28 },
    {  8, 50, 8, 6, 40, 8, 8, 3, 5, { 0xFF, 0xC3, 0xC3, 0xC3, 0xC3, 0xC3, 0xC3, 0xFF }, 34, -34 },
    { 10, 40, 8, 6, 40, 8, 8, 3, 5, { 0xFF, 0xAA, 0xAA, 0xFF, 0xFF, 0xAA, 0xAA, 0xFF }, 28, -28 },
    { 10, 40, 8, 6, 40, 8, 8, 3, 5, { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA }, 28, -28 },
...
void setup() {
  ...
  //digitalWrite(POWER_Q4,LOW);
  pinMode(POWER_BUTTON, INPUT_PULLUP);
  pinMode(POWER_ON_Q4, OUTPUT);

  //pinMode(RUN_BUTTON,OUTPUT);
/*
  while(1)
  {
digitalWrite(RUN_BUTTON,HIGH);
digitalWrite(RUN_BUTTON,LOW);
  }
*/
...
void loop(void) {
  ...
  //while (waitForTouch() > 0) On_Off();

  //Serial.printf("left=%d right=%d\r\n", digitalRead(RUN_BUTTON), digitalRead(AUTO_BUTTON));
  ...
  if ((noBricks(game, &state) && level < GAMES_NUMBER)||(change_level==1)) {
    level++;
    //change_level=0;
...
void setupWall(game_type* game, game_state_type* state) {
  ...
  int colors[] = { RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE, WHITE, BROWN };
...
void drawPlayer(game_type* game, game_state_type* state) {
  // paint
  tft.fillRect(state->playerx, state->bottom, game->playerwidth, game->playerheight, WHITE);//YELLOW
...
void touchToStart() {
  drawBoxedString(50, 200, "BREAKOUT", 3, YELLOW, BLACK);
  drawBoxedString(37, 240, "PRESS TO START", 2, GREEN, BLACK);
  while (waitForTouch() > 0) {}
  while (waitForTouch() < 0) {}
}

void gameOverTouchToStart() {
  drawBoxedString(40, 180, "GAME OVER", 3, RED, BLACK);
  drawBoxedString(37, 220, "PRESS TO START", 2, GREEN, BLACK);
  while (waitForTouch() > 0) {}
  while (waitForTouch() < 0) {}
}
...
void updateScore(int score) {
  ...
  drawBoxedString(tft.width() - 50, 7, buffer, 2, YELLOW, PRIMARY_DARK_COLOR);
}

void updateLevel(int level) {
  char buffer[5];
  snprintf(buffer, sizeof(buffer), levelFormat, level);
  drawBoxedString(tft.width() - 120, 7, buffer, 2, YELLOW, PRIMARY_DARK_COLOR);
}
...
void checkBorderCollision(game_type* game, game_state_type* state, uint16_t x, uint16_t y) {
  ...
  // Kontrola kolízie lopty s hráčom (obdĺžnik vs obdĺžnik)
  bool isHit = 
    (x < state->playerx + game->playerwidth) &&
    (x + game->ballsize > state->playerx) &&
    (y < state->bottom + game->playerheight) &&
    (y + game->ballsize > state->bottom);

  if (isHit) {
    // Modifikuj velx podľa zóny zásahu
    if (x > (state->playerx + game->playerwidth - 3)) {
      state->velx -= 1;
    } else if (x < state->playerx + 3) {
      state->velx += 1;
    }

    // Odráž loptičku nahor
    state->vely = -abs(state->vely);
  }
}
...
int waitForTouch() {
  On_Off();
  if (!digitalRead(RUN_BUTTON) || !digitalRead(AUTO_BUTTON))
    return 1;
  //if (!digitalRead(UP_BUTTON)) {if (level<13) level++; change_level=1; updateLevel(level);  delay(300);}
  //if (!digitalRead(DOWN_BUTTON)) {if (level>1) level--; change_level=1; updateLevel(level); delay(300);}
  return -1;
}

void On_Off(void) {
  if ((!digitalRead (POWER_BUTTON))&&(status_power==0)) {digitalWrite(POWER_ON_Q4, HIGH); status_power=1;}
     else {if ((!digitalRead (POWER_BUTTON))&&(status_power==1)) {digitalWrite(POWER_ON_Q4, LOW); delay(300); while (!digitalRead (POWER_BUTTON));status_power==0;delay(2000);}}
}
